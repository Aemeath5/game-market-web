...
const orders = [
  ...
  { id: 'GM202608070030', buyer: '枫丹来客', seller: '晨曦酒庄', item: '最初的大魔术', amount: '¥ 2,680', created: '08-07 07:48', status: '人工复核' },
...
<template>
  <div class="admin-page">
    ...
    <section class="admin-order-summary">
      ...
      <article><CircleDollarSign /><span><small>今日成交</small><strong>¥ 38,420</strong></span></article>
    </section>
    ...
    <section class="admin-card">
      ...
      <div class="admin-table-wrap">
        <table class="admin-table">
          <thead><tr><th>订单号</th><th>买家</th><th>卖家</th><th>商品</th><th>金额</th><th>下单时间</th><th>状态</th><th>操作</th></tr></thead>
          <tbody><tr v-for="order in orders" :key="order.id">
            <td><strong>{{ order.id }}</strong></td><td>{{ order.buyer }}</td><td>{{ order.seller }}</td><td>{{ order.item }}</td><td><strong>{{ order.amount }}</strong></td><td>{{ order.created }}</td>
